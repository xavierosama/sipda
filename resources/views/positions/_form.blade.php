...
<div class="space-y-5">
    <div>
        <label for="name" class="block text-sm font-semibold text-slate-700">Nama Jabatan</label>
        <input
            type="text"
            id="name"
            name="name"
    ...
    <div>
        ...
        <select
            ...
            required
        >
            <option value="active" @selected(old('status', $position->status ?? 'active') === 'active')>Active</option>
            <option value="inactive" @selected(old('status', $position->status ?? 'active') === 'inactive')>Inactive</option>
        </select>
        @error('status')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
        @enderror
